function FFSuggest() {
	...
	this.handleClick = function() {
		if ($('li.' + pSuggestHighlightClass).length > 0) {
			pCurrentSelection = $('li.' + pSuggestHighlightClass)[0].id
					.replace(pLayerName + '_', '');
		}
	...
	this.handleMouseMove = function(id) {
		unmarkAll();
		$('#' + pLayerName + ' li[id="' + id + '"]').addClass(
				pSuggestHighlightClass);
	}

	this.handleMouseOut = function(id) {
		$('#' + pLayerName + ' li[id="' + id + '"]').removeClass(
				pSuggestHighlightClass);
		pCurrentSelection = -1;
	}
	...
	function checkTab(evt) {
		...
		if (keyCode == 9) {
			...
			if (ovlEl.length > 0) {
				$('input[name=' + pQueryParamName + ']').attr('value',
						ovlEl.attr('value'));
				return false;
			}
		}
	...
	function moveUp() {
		if ($('#' + pLayerName).is(':visible')) {
			if ($('li.' + pSuggestHighlightClass).length == 0) {
				$('li.' + pSuggestRowClass + ':last').addClass(
						pSuggestHighlightClass);
			} else {
				var listWithSelection = $('li.' + pSuggestHighlightClass)
						.parent('ul');
				var prevEl = $('li.' + pSuggestHighlightClass).prevAll(
						'li.' + pSuggestRowClass);
				if (prevEl.length == 0) {
					$('li.' + pSuggestHighlightClass).removeClass(
							pSuggestHighlightClass);
					listWithSelection.find('li.' + pSuggestRowClass + ':last')
	...
	function moveDown() {
		if ($('#' + pLayerName).is(':visible')) {
			if ($('li.' + pSuggestHighlightClass).length == 0) {
				$('li.' + pSuggestRowClass + ':first').addClass(
						pSuggestHighlightClass);
			} else {
				var listWithSelection = $('li.' + pSuggestHighlightClass)
						.parent('ul');
				var nxtEl = $('li.' + pSuggestHighlightClass).nextAll(
						'li.' + pSuggestRowClass);
				if (nxtEl.length == 0) {
					$('li.' + pSuggestHighlightClass).removeClass(
							pSuggestHighlightClass);
					listWithSelection.find('li.' + pSuggestRowClass + ':first')
							.addClass(pSuggestHighlightClass);
				} else {
					$('li.' + pSuggestHighlightClass).removeClass(
							pSuggestHighlightClass);
	...
	function getCurrentSelection() {
		...
		if (highlightId) {
			selection = highlightId.slice(highlightId.lastIndexOf('_') + 1,
					highlightId.length);
		}
		return selection;
	}
	...
	function hideLayer() {
		...
		var ovlEl = $('input#' + pQueryParamName + 'Underlay');
		if (ovlEl.length > 0) {
			$('input[name=' + pQueryParamName + ']').css('background-color',
					ovlEl.css('background-color'));
		}
	}
	...
	function unmarkAll() {
		$('li.' + pSuggestHighlightClass).each(
				function(i) {
	...
	function addParam(url, paramName, paramValue) {
		if (paramName && paramValue) {
			var newParam = encodeURIComponent(paramName) + '='
				...
				function(jsonObj, textStatus) {
					...
					if (suggestions != null && suggestions.length > 0) {
						...
						var outputText = '<div class="' + pLayerName
								+ 'Block clearfix" onMouseDown="'
								+ pInstanceName + '.handleClick();">';
						...
						// if the query contains regular expression
						// metacharacters then escape them.
						query = query.replace(/[^a-zA-Z0-9]/g, "\\$&");
						for (var i = 0; i < suggestions.length; i++) {

							var suggestQuery = suggestions[i].name;
							if (tabWord == '') {
								if (suggestQuery.toLowerCase().indexOf(
										query.toLowerCase()) == 0) {
									tabWord = query
											+ suggestQuery
							...
							if (suggestType != lastType) {
								var suggestTypeHead = ptranslation[suggestType
										+ '.headline'];
								if (!suggestTypeHead) {
									suggestTypeHead = suggestType;
								}
								outputTextTemp += '<li class="suggestHeader">'
										+ suggestTypeHead + '</li>';
							...
							if (suggestType == 'productName') {
								// outputText += getSuggestHtml_productName(id,
								// query, suggestions[i]);
								outputTextImg += outputTextTemp;
								outputTextImg += getSuggestHtml_productName(id,
										query, suggestions[i]);
							} else {
								// outputText += getSuggestHtml_fallback(id,
								// query, suggestions[i]);
								outputTextText += outputTextTemp;
								outputTextText += getSuggestHtml_fallback(id,
										query, suggestions[i]);
							}
							var outputTextTemp = '';
						...
						// outputText += '</ul>';
						if (outputTextText != '') {
							outputText += '<ul class="' + pLayerName
									+ 'BlockText">' + outputTextText + '</ul>';
						} else {
							blockImgBorder = 'noborder';
						}
						if (outputTextImg != '') {
							outputText += '<ul class="' + pLayerName
									+ 'BlockImage ' + blockImgBorder + ' ">'
									+ outputTextImg + '</ul>';
						}
						...
						if (ovlEl.length > 0) {
							...
							if (tabWord) {
								$('input[name=' + pQueryParamName + ']').css(
	...
	function getSuggestHtml_fallback(id, query, jsonSug) {
		...
		var html = '<li id="'
				...
				+ '" class="'
				+ pSuggestRowClass
				+ ' suggestRowWithoutImage" onMouseMove="'
				+ pInstanceName
				+ '.handleMouseMove(\''
				+ id
				+ '\');"'
				...
				+ pSuggestAmountClass + '">' + suggestCount + '</span>';
		return html;
	}
	...
	function getSuggestHtml_productName(id, query, jsonSug) {
		...
		// this is needed to later remove the image tag from the DOM, if the
		...
		var html = '<li id="' + id + '" class="' + pSuggestRowClass + ' '
				...
				+ pInstanceName + '.handleMouseOut(\'' + id + '\');">'
	...
	function moveHorizontalInSuggestBox() {
		var numBlockTextElements = $('ul.' + pSuggestLayerBlockText + ' li.'
				+ pSuggestRowClass).length;
		var numBlockImageElements = $('ul.' + pSuggestLayerBlockImage + ' li.'
				+ pSuggestRowClass).length;
		...
		if (numBlockTextElements > 0 && numBlockImageElements > 0) {
			...
			if (!isHighlighted) {
				$(
						'ul.' + pSuggestLayerBlockImage + ' li.'
								+ pSuggestRowClass + ':first').addClass(
						pSuggestHighlightClass);
			} else if (isHighlighted) {
				var isInBlockImage = $('ul.' + pSuggestLayerBlockImage + ' li.'
						+ pSuggestHighlightClass).length > 0;
				$('li.' + pSuggestHighlightClass).removeClass(
						pSuggestHighlightClass);
				if (isInBlockImage) {
					$(
							'ul.' + pSuggestLayerBlockText + ' li.'
									+ pSuggestRowClass + ':first').addClass(
							pSuggestHighlightClass);
				} else {
					$(
